...
public class Board extends JPanel implements ActionListener, IGameClient {
	...
	public void paint(Graphics g) {
		...
		if (gameover) {
			...
			if (connector == null || connector.getGameStage() == GameStage.FINISHED) {
				drawCenteredText(g2d, "PRESS SPACE TO RESTART", getHeight() * 3/4);
			}
			if (connector != null && connector.getGameStage() == GameStage.FINISHED) {
				if (connector.areWeTheWinner()) {
					drawCenteredText(g2d, "YOU HAVE WON!", getHeight() * .4f, bigfnt);
				} else {
					drawCenteredText(g2d, "YOU HAVE LOST TO " + connector.getWinnersName(), getHeight() * .4f, bigfnt);
				}
			}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (connector != null && connector.getGameStage() != GameStage.IN_PROGRESS) {
			// Do nothing
			this.repaint();
			return;
		}
	...
	private class TAdapter extends KeyAdapter {
		...
		public void keyPressed(KeyEvent e) {
			...
			else if (crawler != null && !isPlayerDead())
				crawler.keyPressed(e, crawlerzoffset);

			if (gameover) {
				if (key == KeyEvent.VK_SPACE) {
					// Check all the players have finished 
					if (connector != null) {
						if (connector.getGameStage() != GameStage.FINISHED) {
							return;
						}
					}
					startGame();
	...
	public void dataReceivedByTCP(int fromplayerid, int key, int value) {
		try {
			...
			if (key == CODE_LEVEL_NUMBER) {
				multiplayer_msg = player + " has made it to level " + value;
	...
	public void error(int error_code, String msg) {
		multiplayer_msg = "Error: " + msg;

	}

	@Override
	public void gameEnded(String winner) {
		multiplayer_msg = "Game Finished!";
		gameover = true;
	}
